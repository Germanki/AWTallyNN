...
png(filename = "/home/ubuntu/AWTallyNN/tallynn/R/", format(date, "%Y-%m-%d"), "_filtering_summary.png", width = 800, height = 600, res = 300)
plotFiltering(output, valueType = "reads", onlyActiveFilters = TRUE)
dev.off()

# Plot the error statistics and save as PNG
png(filename = "/home/ubuntu/AWTallyNN/tallynn/R/", format(date, "%Y-%m-%d"), "_error_stats.png", width = 800, height = 600, res = 300)
plotErrorStats(output)
dev.off()

# Plot the distribution of variant counts and save as PNG
png(filename = "/home/ubuntu/AWTallyNN/tallynn/R/", format(date, "%Y-%m-%d"), "_variant_distribution.png", width = 800, height = 600, res = 300)
plotDistributions(output, plotType = "density", pseudocount = 1)
dev.off()

# Plot the total counts per variant and save as PNG
png(filename = "/home/ubuntu/AWTallyNN/tallynn/R/", format(date, "%Y-%m-%d"), "_variant_totals.png", width = 800, height = 600, res = 300)
plotTotals(output)
dev.off()
